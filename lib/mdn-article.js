...
const { AllHtmlEntities: Entities } = htmlEntities;
...
export const getArticleMetadata = async (url) => {
  ...
  const IMAGE_REGEX = /<meta property="og:image" content="(.*?)"\/>/i;
  ...
  metadata.title = entities.decode(metadata.title);
  metadata.description = entities.decode(metadata.description);
